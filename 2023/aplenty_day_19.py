...
@dataclass(slots=True)
class PartRange:
    x: tuple[int, int] = (1, 4000)  # Sets defualt value
...
class Step():
    ...
    def split(self, part_range):  # The first one will be the one that satisfies the condition
        copy_values_true, copy_values_false = asdict(
            part_range), asdict(part_range)

        if self.comparison == lt:
            copy_values_true[self.var], copy_values_false[self.var] = (
                copy_values_true[self.var][0], self.value-1), (self.value, copy_values_true[self.var][1])
        else:
            copy_values_true[self.var], copy_values_false[self.var] = (
                self.value+1, copy_values_true[self.var][1]), (copy_values_true[self.var][0], self.value)

        part_range_true, part_range_false = PartRange(
            **copy_values_true), PartRange(**copy_values_false)
    ...
    def check_range(self) -> int:
        ...
        while len(ranges_queue) > 0:
            ...
            if current_workflow == "A":
                ...
                ranges_queue.extend(
                    self.workflows[current_workflow].check_ranges(current_range))
        return count
...
def string_2_parts(part_strings):
    d = [{e.split('=')[0]: int(e.split('=')[1])
          for e in part[1:-1].split(',')} for part in part_strings]
    return [Part(**values) for values in d]


def string_2_workflows(workflows_string: str):
    workflows = []
...
def check_parts(filename: str) -> int:
...
def check_ranges(filename: str) -> int:
    '''This function checks which ranges lead to being accepted'''
